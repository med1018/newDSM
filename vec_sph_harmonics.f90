...
!########################################################################################
    subroutine comp_vec_sph_harmonics(R,S,T,l_max,m_max,theta,phi)
        ...
        integer :: l, m
        double precision :: P(0:m_max,0:l_max), dP, c, mathL
        complex(kind(0d0)) :: z
        ...
        if ( theta > threshold_theta ) then
            ...
            do l = 1, l_max
                ...
                do m = 0, min(l, m_max)
                    ...
                    R(1,m,l) = c * P(m,l)
                    S(2,m,l) = c * dP / mathL
                    S(3,m,l) = im * dble(m) * R(1,m,l) / sin(theta) / mathL
        ...
        elseif ( theta >= 0 .and. theta <= threshold_theta ) then
            ...
            do l = 0, l_max
                ...
                R(1,0,l) =   c * (1.0d0 - ( mathL * theta ) ** 2 / 4d0 )
                S(2,0,l) = - c * mathL * theta / 2d0
                ...
                do m = 1, min(l, m_max)
                    ...
                    R(1,m,l) = c * theta ** m
                    S(2,m,l) = c * ( theta ** (m-1) ) * dble(m) / mathL
                    S(3,m,l) = im * S(2,m,l)
    ...
    subroutine assoc_Legendre_plys(P,l_max,m_max,x)
        ...
        do m = 0, m_max
            ...
            do l = m, l_max-1
                ...
                if (l /= 0) P(m,l+1) = ( dble(2*l+1) * x * P(m,l) - dble(l+m) * P(m,l-1) ) / dble(l-m+1)

            enddo

            if (m /= m_max) P(m+1,m+1) = dble(2*m+1) * s * P(m,m)
